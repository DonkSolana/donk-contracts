...
import UniswapV2Pair from '@oneverseswap/core/build/UniswapV2Pair.json'
...
describe('PitBreeder', () => {
  ...
  beforeEach(async function() {
    ...
    pairs = {
      'viper/weth': await createLpToken(
        alice,
        factory,
        pairFactory,
        govToken,
      ...
      'viper/busd': await createLpToken(
        alice,
        factory,
        pairFactory,
        govToken,
  ...
  describe('setBridge', function() {
    it('does not allow to set bridge for Governance Token', async function() {
      await expect(pitBreeder.setBridge(govToken.address, weth.address)).to.be.revertedWith(
        'PitBreeder: Invalid bridge'
      )
    })

    it('does not allow to set bridge for WETH', async function() {
      await expect(pitBreeder.setBridge(weth.address, govToken.address)).to.be.revertedWith(
        'PitBreeder: Invalid bridge'
      )
    })
  ...
  describe('convert', function() {
    ...
    it('reverts if pair does not exist', async function() {
      this.timeout(0)
      await expect(pitBreeder.convert(link.address, pairs['dai/link'].address)).to.be.revertedWith(
        'PitBreeder: Invalid pair'
      )
    })
